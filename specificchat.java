...
public class specificchat extends AppCompatActivity {
    ...
    private void setupListeners() {
        ...
        // Back button
        mbackbuttonofspecificchat.setOnClickListener(view -> onBackPressed());

        // Toolbar click - show user profile or status
        mtoolbarofspecificchat.setOnClickListener(view -> {
            // Could show user profile or last seen status
            Toast.makeText(this, "Click on name to view profile", Toast.LENGTH_SHORT).show();
        });
    }
    ...
    private void setupMessagesListener() {
        ...
        messagesListener = new ChildEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                ...
                Toast.makeText(specificchat.this, "Failed to load messages: " + error.getMessage(), Toast.LENGTH_LONG).show();
            }
        };
    ...
    private void updateUserStatus(String status) {
        FirebaseUser currentUser = firebaseAuth.getCurrentUser();
        if (currentUser != null) {
            // Update in both databases for consistency
            
            // Update in Firestore
            DocumentReference firestoreRef = firebaseFirestore.collection("Users")
                    .document(currentUser.getUid());
            firestoreRef.update("status", status)
                    .addOnFailureListener(e -> Log.e(TAG, "Firestore status update failed: " + e.getMessage()));

            // Update in Realtime Database
            DatabaseReference realtimeRef = firebaseDatabase.getReference("Users")
                    .child(currentUser.getUid())
                    .child("status");
            realtimeRef.setValue(status)
                    .addOnFailureListener(e -> Log.e(TAG, "Realtime DB status update failed: " + e.getMessage()));
        }
    }

    @Override
    public void onBackPressed() {
        // Clean up listeners before going back
        cleanupListeners();
        // Update status to online when going back to chat list
        updateUserStatus("Online");
        super.onBackPressed();
    }
}